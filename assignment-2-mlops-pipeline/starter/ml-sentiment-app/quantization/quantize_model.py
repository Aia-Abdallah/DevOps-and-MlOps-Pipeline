...
def quantize_gptq(
    ...
    # ✅ Fix: use Salesforce/wikitext instead of wikitext
    config = "wikitext-2-raw-v1" if dataset_name == "wikitext" else None
    dataset_name_hf = "Salesforce/wikitext" if dataset_name == "wikitext" else dataset_name
    dataset = load_dataset(dataset_name_hf, config, split=f"train[:{num_samples * 4}]")
    ...
    if os.path.exists(config_path):
        ...
        if config.get("dtype") == "bfloat16" or config.get("torch_dtype") == "bfloat16":
            ...
            print("  Patched config.json: dtype bfloat16 → float16")
...
def evaluate_quantized_model(
    ...
    """Evaluate a quantized model and log metrics to MLflow."""
    import csv
    ...
    print(f"  Avg latency: {metrics['avg_latency_s']:.3f}s, size: {metrics['quantized_size_gb']:.2f} GB")
    ...
    if mlflow_uri:
        ...
        with mlflow.start_run(run_name="quantization_eval"):
            ...
            print(f"  Metrics logged to MLflow experiment: {experiment_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="Qwen/Qwen2.5-1.5B", help="Model to quantize")
    ...
    parser.add_argument("--evaluate", action="store_true", help="Evaluate after quantization")
    parser.add_argument("--eval-dataset", default=None, help="Path to evaluation CSV")
